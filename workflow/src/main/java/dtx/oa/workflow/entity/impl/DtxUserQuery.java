...
public class DtxUserQuery extends AbstractQuery<UserQuery, User> implements UserQuery{
    ...
    private List<User> getUsers(){
        ...
        if(roleId!=null&&!"".equals(roleId)){
           List<RoleUser> rus=IDaoFactory.iRoleUserDao().queryByRole(IDaoFactory.iRoleDao().getRoleById(roleId));
           users=new ArrayList<>();
           for(RoleUser ru:rus)
               users.add(IDaoFactory.iUserDao().getUserById(ru.getUser().getUuid()));
           rus=null;
        }else
